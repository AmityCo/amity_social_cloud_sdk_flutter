...
const projectValidator = require('./sample-code-project-validator.js')
...
async function main() {
    ...
    if (!args.includes('-skip-project-validation')) {
        try {
            await projectValidator.validate()
        } catch(error) {
            // If we fail to validate the sample code project, we stop here.
            console.log(error)
            return
        }
    }
...
async function pushSampleCodeUpdate(sampleCode) {
    ...
    const git_url = `https://${process.env.GITHUB_USERNAME}:${process.env.GITHUB_PERSONAL_ACCESS_TOKEN}@gist.github.com/${gist_id}.git`

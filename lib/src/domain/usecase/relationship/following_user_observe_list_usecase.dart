...
class FollowingUserObserveListUseCase
  ...
  final FollowingUserComposerUsecase followingUserComposerUsecase;

  FollowingUserObserveListUseCase(
      {required this.followRepo,
      required this.pagingIdRepo,
      required this.userRepo,
      required this.followingUserComposerUsecase});
  ...
  void _onChanges(
    ...
    if (followEntities.isEmpty || pagingIds.isEmpty) {
      ...
      await Stream.fromIterable(follows).forEach((element) async {
        element = await followingUserComposerUsecase.get(element);
      });
      if (!streamController.isClosed) {
